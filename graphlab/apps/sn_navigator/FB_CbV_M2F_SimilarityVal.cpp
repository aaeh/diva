     ...
     vdata() :
      ...
      explicit vdata(std::vector<graphlab::vertex_id_type> f,std::vector< std::string> a_n, std::multimap<std::string, std::string> a_v,  bool isc) :
        ...
        void save(graphlab::oarchive& oarc) const {
                ...
                num = CAG_supp.size();
                oarc << num;
                for(std::vector<std::double>::const_iterator iter = CAG_Supp.begin();
...
public:
   ...
   void apply(icontext_type& context, vertex_type& vertex,const gather_type& total) {
        ...
        else if(context.iteration() == 2 && vertex.data().friends.size()>0 ){
...
/////////////////////////////////////////////////Load data    
   ...
   while(std::getline(ss4, token, '\')){
      att_values.insert(std::make_pair("education",token));
      /*int index =0;
      std::string cat;
      ...
      while(std::getline(ss10, token_sub, '@')){
         if(index ==0){
            ...
            index ++;
             ...
             att_values.insert(std::make_pair("interest",cat + "#" + token_sub));
