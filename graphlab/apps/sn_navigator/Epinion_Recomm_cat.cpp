                ...
                for(size_t p1=0; p1<len; p1++){
                    for(size_t p2=p1+1; p2<len; p2++){
                       ...
                       if((find(val_n2, value1) >=0) && (find(val_n2, value2) >=0)){
                              ...
                              c1 << value1;
                              c2 << value2;
                              key =  "cat:" + c1.str() + " & cat:" + c2.str();
                              key1 = "cat:" + c2.str() + " & cat:" + c1.str();
                              int index =  findS(vertex.data().att_group, key);
                              int index1 = findS(vertex.data().att_group, key1);
                              if(index ==-1 && index1==-1){
...
//######################################## main function #########################################//
  int main(int argc, char** argv) {
   ...
   engine2.signal_all();
   engine2.start();
   graph.save("EpRec_Data_cat.txt", graph_writer(),
             false,    // do not gzip
             true,     // save vertices
